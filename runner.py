        ...
        for rule in rules:
            ...
            if matched and extract_cfg:
                ...
                for output_key, template in extract_cfg.items():
                    # 应用模板替换
                    value = self._apply_template(template, context)
                    if not value:
                        continue
                    ...
                    if output_key in filters:
                        ...
                        # if_not_empty: 值为空时跳过
                        if filter_cfg.get('if_not_empty') and not value:
                            continue
                        
                        # if_contains: 不包含指定文本时跳过
                        if_contains = filter_cfg.get('if_contains')
                        if if_contains and if_contains not in value:
                            continue
...
class Orchestrator:
    ...
    def _print_event_flow(self):
        ...
        def trace_chain(event_type, indent=0, visited=None):
            ...
            logging.info(f"{prefix}📍 {event_type}{producer_str}")
            
            # 递归打印下游
            for consumer in consumers:
                # 找到这个消费者的输出
                for mod in all_modules:
                    if mod.name == consumer:
                        for output_type in mod.outputs:
                            trace_chain(output_type, indent + 1, visited.copy())
                        break
        
        for root_event in sorted(root_events):
